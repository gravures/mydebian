---
name: python-typing
description: Python type safety with type hints, generics, protocols, and strict type checking. Use when adding type annotations, implementing generic classes, defining structural interfaces.
---

# Python Type Safety

Leverage Python's type system to catch errors at static analysis time.
Type annotations serve as enforced documentation that tooling validates automatically.
...
**Note:** The `type` statement was introduced in Python 3.10 for simple aliases.
Generic type statements require Python 3.12+.
...
3. **Use generics** - Preserve type info in reusable code
4. **Define protocols** - Structural typing for interfaces
5. **Narrow types** - Use guards to help the type checker
6. **Bound type vars** - Restrict generics to meaningful types
7. **Create type aliases** - Meaningful names for complex types
8. **Minimize `Any`** - Use specific types or generics. `Any` is acceptable
     for truly dynamic data or when interfacing with untyped third-party code
9. **Document with types** - Types are enforceable documentation
